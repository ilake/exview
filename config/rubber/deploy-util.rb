  ...
  namespace :util do
    ...
    task :backup do
      ...
      task task_name, :hosts => selected_db_instance.full_name do
        rsudo "cd #{current_path} && RUBBER_ENV=#{RUBBER_ENV} BACKUP_DIR=/opt/db_backups DBUSER=#{rubber_env.db_user} DBPASS=#{rubber_env.db_pass} DBNAME=#{rubber_env.db_name} DBHOST=#{selected_db_instance.full_name} rake rubber:backup_db"
      end
      send task_name
    end
    ...
    task :restore_s3 do
      ...
      for instance in master_instances+slaves
        ...
        task task_name, :hosts => instance.full_name do
          rsudo "cd #{current_path} && RUBBER_ENV=#{RUBBER_ENV} BACKUP_DIR=/opt/db_backups DBUSER=#{rubber_env.db_user} DBPASS=#{rubber_env.db_pass} DBNAME=#{rubber_env.db_name} DBHOST=#{instance.full_name} rake rubber:restore_db_s3"
        end
        send task_name
      end
    end
